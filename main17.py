...
def generate_response(query):
    ...
    if 'choices' in response_text and len(response_text['choices']) > 0:
        ...
        chatbot = ChatBot('Bookworm')
        trainer = ChatterBotCorpusTrainer(chatbot)
        trainer.train('chatterbot.corpus.english')
        return chatbot.get_response(query).text
...
def on_send_click():
    ...
    if query:
        ...
        if feedback:
            ...
            web_response = generate_response(query)
            chat_history.insert(tk.END, f'Bot: {web_response} (Web)\n')
            clf.partial_fit(vectorizer.transform([query.lower()]), [web_response])
            data = data.append({'queries': query.lower(), 'answers': web_response}, ignore_index=True)
